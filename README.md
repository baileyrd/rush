...
| Builtins | ✅ | `cd`, `pwd`, `echo`, `export`, `unset`, `test`/`[ ]`, `true`, `false`, `:`, `break`/`continue`/`return`, `exit`, `alias`/`unalias`, `set`, `trap`, `read`/`mapfile`/`readarray`, `printf`, `shift`, `local`, `declare`/`typeset`, `readonly`, `getopts`, `command`, `type`, `hash`, `.`/`source`, `eval` (+ `jobs`/`fg`/`bg`/`kill`/`wait`/`exec`/`umask`/`ulimit` on Unix) |
...
| Variable expansion (`$VAR`, `~`, `$(...)`) | ✅ | `$VAR`, `${VAR}`, `$?`, `$$`/`$PPID`/`$-`, `${V:-def}`/`:=`/`:+`/`:?`, `${#V}`, `${V#pat}`/`##`/`%`/`%%` (prefix/suffix pattern removal), `${V/pat/repl}` (`//`, `/#`, `/%`), `${V:off:len}` substrings & array slices, `${V^^}`/`${V,,}` case conversion, `${!v}` indirection, `${!prefix@}` name listing, `${V@Q}`/`@E`/`@a`/`@A` transforms, tilde, command substitution; unquoted results field-split on `$IFS` |
