...
| Variable expansion (`$VAR`, `~`, `$(...)`) | ✅ | `$VAR`, `${VAR}`, `$?`, `$$`/`$PPID`/`$-`, `${V:-def}`/`:=`/`:+`/`:?`, `${#V}`, `${V#pat}`/`##`/`%`/`%%` (prefix/suffix pattern removal), `${V/pat/repl}` (`//`, `/#`, `/%`), `${V:off:len}` substrings & array slices, `${V^^}`/`${V,,}` case conversion, tilde, command substitution; unquoted results field-split on `$IFS` |
