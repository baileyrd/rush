...
| Globbing (`*`, `?`, `[…]`) | ✅ | hand-rolled matcher; ranges, `[!…]`, POSIX named classes (`[[:alpha:]]`, `[[:digit:]]`, …), extended globs (`@(a|b)`, `!(pat)`, `+(pat)`, …— always on), multi-component (`src/*.rs`); dotfiles skipped unless pattern starts with `.` |
